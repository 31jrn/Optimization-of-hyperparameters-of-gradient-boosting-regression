...
def load_data(file_path, x_cols=None, y_col=None):
    ...
    if not np.issubdtype(y.dtype, np.number):
        try:
            ...
            raise ValueError(f"Целевая переменная '{y_col}' содержит нечисловые значения, которые не удалось преобразовать: {e}")
...
def split_data(X, y, test_size=0.2, val_size=0.2, random_state=42):
    ...
    return (X_train, X_test, y_train, y_test), (X_train_train, X_train_val, y_train_train, y_train_val)
...
def main():
    ...
    DATA_PATH = 'data.xlsx'          # путь к файлу Excel
    TEST_SIZE = 0.2
    ...
    X, y = load_data("C:/Users/vanya/Desktop/Мазур-данные для регрессии.xlsx")
    
    # 2. Разделение
    (X_train, X_test, y_train, y_test), (X_train_train, X_train_val, y_train_train, y_train_val) = split_data(
    ...
    custom_cv = [(train_indices, val_indices)]  # список из одного кортежа (train_idx, val_idx)
    ...
    final_model_grid = xgb.XGBRegressor(**best_params_grid, objective='reg:squarederror', random_state=RANDOM_STATE)
    ...
    final_model_random = xgb.XGBRegressor(**best_params_random, objective='reg:squarederror', random_state=RANDOM_STATE)
    final_model_random.fit(X_train, y_train)
    rmse_random, mape_random = evaluate_model(final_model_random, X_test, y_test)
    results['no_cv']['random'] = {'rmse': rmse_random, 'mape': mape_random, 'time': time_random}
    ...
    final_model_bayes = xgb.XGBRegressor(**best_params_bayes, objective='reg:squarederror', random_state=RANDOM_STATE)
    final_model_bayes.fit(X_train, y_train)
    rmse_bayes, mape_bayes = evaluate_model(final_model_bayes, X_test, y_test)
    results['no_cv']['bayes'] = {'rmse': rmse_bayes, 'mape': mape_bayes, 'time': time_bayes}
    ...
    final_model_grid_cv = xgb.XGBRegressor(**best_params_grid_cv, objective='reg:squarederror', random_state=RANDOM_STATE)
    final_model_grid_cv.fit(X_train, y_train)
    rmse_grid_cv, mape_grid_cv = evaluate_model(final_model_grid_cv, X_test, y_test)
    results['with_cv']['grid'] = {'rmse': rmse_grid_cv, 'mape': mape_grid_cv, 'time': time_grid_cv}
    ...
    final_model_random_cv = xgb.XGBRegressor(**best_params_random_cv, objective='reg:squarederror', random_state=RANDOM_STATE)
    final_model_random_cv.fit(X_train, y_train)
    rmse_random_cv, mape_random_cv = evaluate_model(final_model_random_cv, X_test, y_test)
    results['with_cv']['random'] = {'rmse': rmse_random_cv, 'mape': mape_random_cv, 'time': time_random_cv}
    ...
    final_model_bayes_cv = xgb.XGBRegressor(**best_params_bayes_cv, objective='reg:squarederror', random_state=RANDOM_STATE)
    final_model_bayes_cv.fit(X_train, y_train)
    rmse_bayes_cv, mape_bayes_cv = evaluate_model(final_model_bayes_cv, X_test, y_test)
    results['with_cv']['bayes'] = {'rmse': rmse_bayes_cv, 'mape': mape_bayes_cv, 'time': time_bayes_cv}
    

    columns = ['Характеристика', 'Сеточный поиск', 'Случайный поиск', 'Байесовский поиск']
